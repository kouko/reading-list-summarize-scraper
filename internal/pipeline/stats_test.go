...
func TestClassifyResult(t *testing.T) {
	...
	tests := []struct {
		...
		{"wrapped partial (summarization failed under content)", fmt.Errorf("%w: summarize: %v", errPartial, cause), bucketPartial},
...
func TestStats_PartialKeptSeparateFromSuccessAndFailed(t *testing.T) {
	...
	// partial (content extracted, summary failed) must NOT inflate Success
	// nor mix into Failed.
	s := &Stats{}
	for _, err := range []error{
		nil,        // success
		errSkipped, // skipped
		fmt.Errorf("%w: summarize: quota", errPartial), // partial
		fmt.Errorf("extract: empty content"),           // failed
	} {
