...
func TestCircuitBreaker_RepeatedQuotaFailureWhileOpen_RestartsCooldown(t *testing.T) {
	now := time.Now()
	cb := newCircuitBreaker("test", 1, 300*time.Second)
	cb.nowFunc = func() time.Time { return now }

	cb.RecordQuotaFailure(0, KindExhausted) // open, 300s cooldown
	if cb.State() != stateOpen {
		t.Fatal("should open on first quota failure")
	}

	// 30s later another quota failure arrives while still open.
	now = now.Add(30 * time.Second)
	cb.RecordQuotaFailure(0, KindExhausted)
	if cb.State() != stateOpen {
		t.Error("should remain open after a repeated quota failure")
	}

	// The cooldown window restarts from the most recent failure: 299s after
	// the second failure (329s after the first) is still within cooldown.
	now = now.Add(299 * time.Second)
	if cb.Allow() {
		t.Error("cooldown should restart from the most recent failure, still blocked")
	}
	now = now.Add(2 * time.Second)
	if !cb.Allow() {
		t.Error("should probe once cooldown elapses from the most recent failure")
	}
}
...
func TestCircuitBreaker_HalfOpenEmptyBelowThreshold_ReArms(t *testing.T) {
	now := time.Now()
	cb := newCircuitBreaker("test", 1, 5*time.Minute)
	cb.emptyThreshold = 3
	cb.nowFunc = func() time.Time { return now }

	// Open via a QUOTA failure (not empties), so emptyCount stays 0 and the
	// half-open empty probe below is genuinely below the empty threshold.
	cb.RecordQuotaFailure(0, KindExhausted)
	if cb.State() != stateOpen {
		t.Fatal("should open on quota failure")
	}
	...
	// A single empty probe (emptyCount=1 < threshold 3) must NOT close or
	// strand the circuit — it must re-arm (re-open) so it isn't stuck.
	cb.RecordEmptyResponse()
	if cb.State() != stateOpen {
		t.Fatalf("below-threshold empty probe in half-open must re-arm (re-open), got %v", cb.State())
	}
	if cb.Allow() {
		t.Error("should be cooling down again right after the re-arm")
	}
}
