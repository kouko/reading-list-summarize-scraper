...
func TestParseRetryAfter_Seconds(t *testing.T) {
	...
	// "Retry-After: 0" and negatives must clamp to 0, so chooseCooldown falls
	// through to the kind default rather than reopening instantly.
...
func TestParseRetryAfterFromText(t *testing.T) {
	cases := map[string]time.Duration{
		...
		`{"retryDelay": "1.5s"}`:    1500 * time.Millisecond, // fractional seconds
		"Please retry in 42s":       42 * time.Second,
		"retry after 30 seconds":    30 * time.Second,
		"please retry in 2 minutes": 0, // "minutes" is not a seconds unit → no match
		"no hint here":              0,
