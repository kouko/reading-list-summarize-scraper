...
func TestFallback_QuotaError_ImmediatelySwitchesSameTask(t *testing.T) {
	const task = "the full article text to be summarized"
	opts := SummarizeOptions{Prompt: "PROMPT: summarize the article", MaxTokens: 1500}
	...
	// (2) The switch was immediate within the one call: primary tried exactly
	//     once (no same-provider retry), fallback exactly once.
	if primary.calls != 1 {
	...
	// (3) The SAME task was handed to the fallback — identical text and
