...
// config.example.yaml uses the map shape for openai-compat: a reserved
// `default` instance plus named LAN instances box1/box2. Before the example
// is migrated from the old single-struct block this fails (box1/box2 keys
// absent, or a struct->map parse error).
